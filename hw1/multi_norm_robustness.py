...
def test_model_on_single_attack(model, test_loader, attack="pgd_linf", eps=0.1):
    model.eval()
    tot_test, tot_acc = 0.0, 0.0
...
def evaluate_model_multiple_eps(model, test_loader, attack_type="linf", eps_values=[]):
    model.eval()
    results = {}
...
def plot_history(history):
    ...
    axes[1].plot(history["test_standard_acc"], label="Test Standard Acc")
    axes[1].plot(history["test_robust_acc"], label="Test Robust Acc")
    axes[0].xlabel("Epoch")
    axes[1].xlabel("Epoch")
    axes[0].ylabel("Loss")
...
def hw3_adverserial_training():
    ...
    # linf training
    model_linf = PreActResNet18(10, device=device, activation="softplus1").to(device)
    history_linf = adversarial_training(
        model_linf,
        train_loader,
        test_loader,
        attack_type="linf",
    ...
    plot_history(history_linf)
    results_linf = evaluate_model_multiple_eps(
        model_linf,
        test_loader,
        "linf",
        eps_values=[0.0, 2 / 255, 4 / 255, 8 / 255, 16 / 255, 32 / 255],
    )
    print(f"linf robustness results: {results_linf}")
    results_l2 = evaluate_model_multiple_eps(
        model_linf,
        test_loader,
        "l2",
        eps_values=[0.0, 0.1, 0.25, 0.5, 0.75, 1.0],
    )
    print(f"l2 robustness results: {results_l2}")


if __name__ == "__main__":
    # hw1_test_multi_norm_robustness()
    hw3_adverserial_training()
